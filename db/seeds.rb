...
if User.first( :conditions=>{:admin=>true}).blank?
  ...
  employee = Employee.find_or_create_by_employee_number(:employee_number => 'admin',:joining_date => Date.today,:first_name => 'Admin',:last_name => 'User',
    :employee_department_id => employee_department.id,:employee_grade_id => employee_grade.id,:employee_position_id => employee_position.id,:employee_category_id => employee_category.id,:status => true,:nationality_id =>'76', :date_of_birth => Date.today-365, :email => 'noreply@lindax.net',:gender=> 'm')
...
[
  ...
  {"settings_key" => "FeeSubmissionEnabled"               ,"is_enabled" => false}
].each do |param|
...
#The following code has been moved in to a seed file named "add_reports_view_privilege"
