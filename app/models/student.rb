...
class Student < ActiveRecord::Base
  ...
  named_scope :by_full_name, :order=>'first_name,last_name',:conditions => { :is_active => true }
  ...
  def guardian_name
    guardian_name=self.guardians.first()
    guardian_name.present? ? "#{guardian_name.full_name}"+"(#{guardian_name.relation})" : ""
  end
  ...
  def self.students_fee_defaulters(parameters)
    ...
    if sort_order.nil?
      if transaction_class=="HostelFeeCollection"
        ...
        :joins=>[:hostel_fees] ,
        :conditions=>["hostel_fees.hostel_fee_collection_id=? and hostel_fees.finance_transaction_id is NULL  AND `hostel_fees`.`is_active` = true",fee_collection_id],
        :order=>"balance DESC")
        ...
        students=Student.all(
          ...
          :conditions=>["transport_fees.transport_fee_collection_id=? and transport_fees.transaction_id is NULL AND `transport_fees`.`is_active` = true",fee_collection_id],
          :order=>"balance DESC")
      else
  ...
  def self.student_wise_fee_collections(parameters)
    ...
    if FedenaPlugin.can_access_plugin?("fedena_hostel")
      ...
      :joins => [:hostel_fees],
      :conditions => [
        "`hostel_fee_collections`.`is_deleted` = ? AND `hostel_fees`.`student_id` = ? AND `hostel_fees`.`finance_transaction_id` IS NULL and hostel_fees.rent > ? AND hostel_fee_collections.due_date < ? AND `hostel_fees`.`is_active` = ?",
        false,student_id,0.0,Date.today,true
      ],
      :order => "balance DESC"
    )
    end
    if FedenaPlugin.can_access_plugin?("fedena_transport")
      ...
      :conditions => ["transport_fee_collections.is_deleted = ? AND `transport_fees`.`receiver_id` = ? AND `transport_fees`.`receiver_type` = 'Student' AND `transport_fees`.`transaction_id` IS NULL and transport_fees.bus_fare > ? AND transport_fee_collections.due_date < ? AND `transport_fees`.`is_active` = ?",false,student_id,0.0,Date.today,true],
      :order => "balance DESC")
    end
    data=[]
  ...
  def fees_list
    FinanceFeeCollection.find(:all,
      :joins=>"INNER JOIN fee_collection_batches on fee_collection_batches.finance_fee_collection_id = finance_fee_collections.id
              ...
              INNER JOIN batches on (batches.id = batch_students.batch_id or batches.id= #{self.batch_id})
      ...
      :conditions=>"finance_fees.student_id='#{self.id}'  and
                    ...
                    (finance_fees.batch_id=batch_students.batch_id or finance_fees.batch_id=#{self.batch_id})",
