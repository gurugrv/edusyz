...
class FinanceFeeCollection < ActiveRecord::Base
  ...
  def check_fee_category(batch)
    if FinanceTransaction.find(:all,
      ...
      :conditions=>["finance_fees.fee_collection_id=#{id} and students.batch_id=#{batch} and finance_fees.batch_id=#{batch}"]).present?
      return false
    else
      return true
    end
  ...
  def self.batch_fee_collections(parameters)
    ...
    if FedenaPlugin.can_access_plugin?("fedena_transport")
      fee_collections_transport= TransportFeeCollection.all(
          ...
          :conditions =>[
            "batches.id=? and transport_fees.transaction_id is NULL and receiver_type='Student' and students.id IS NOT NULL and transport_fee_collections.due_date<'#{Date.today}' AND `transport_fees`.`is_active` = ?",
            batch_id,true
            ],
      ...
      fee_collections_hostel=HostelFeeCollection.all(
            ...
            :conditions =>[
              "batches.id=? and hostel_fee_collections.is_deleted=? and hostel_fees.finance_transaction_id IS NULL and hostel_fee_collections.due_date < ? and hostel_fees.rent > ? AND `hostel_fees`.`is_active` = ?",
                batch_id,false,Date.today,0,true
              ],
  ...
  def fine_to_pay(student)
    ...
    fee_particulars = finance_fee_particulars.all(:conditions=>"batch_id=#{financefee.batch_id}").select{|par|  (par.receiver.present?) and (par.receiver==student or par.receiver==financefee.student_category or par.receiver==financefee.batch) }
    discounts=fee_discounts.all(:conditions => "batch_id=#{financefee.batch_id}").select { |par| (par.receiver.present?) and ((par.receiver==financefee.student or par.receiver==financefee.student_category or par.receiver==financefee.batch) and (par.master_receiver_type!='FinanceFeeParticular' or (par.master_receiver_type=='FinanceFeeParticular' and (par.master_receiver.receiver==financefee.student or par.master_receiver.receiver==financefee.student_category or par.master_receiver.receiver==financefee.batch)))) }
