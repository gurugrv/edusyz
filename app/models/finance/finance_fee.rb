...
class FinanceFee < ActiveRecord::Base
  ...
  def self.csv_batch_fees_head_wise_report(parameters)
    ...
    if batch_ids.present?
      students=Student.all(:select => "DISTINCT students.*", :conditions => ["finance_fees.batch_id IN (?)", batch_ids], :joins => [{:finance_fees=>{:finance_fee_collection=>:fee_collection_batches}}], :include => [{:finance_fees => [{:finance_fee_collection => [{:collection_particulars => :finance_fee_particular}, {:collection_discounts => :fee_discount}]}, :finance_transactions, :batch]}], :order => 'first_name ASC')
    else
      students=Student.all(:select => "DISTINCT students.*", :joins => [{:finance_fees=>{:finance_fee_collection=>:fee_collection_batches}}], :include => [{:finance_fees => [{:finance_fee_collection => [{:collection_particulars => :finance_fee_particular}, {:collection_discounts => :fee_discount}]}, :finance_transactions, :batch]}], :order => 'first_name ASC')
    end
