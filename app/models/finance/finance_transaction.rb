...
class FinanceTransaction < ActiveRecord::Base
  ...
  named_scope :reference_no_like, lambda{|query|{:conditions => ["reference_no LIKE ?","%#{query}%"]}}
  named_scope :start_date_as, lambda{|query|{:conditions => ["transaction_date >= ?","#{query}"]}}
  named_scope :end_date_as, lambda{|query|{:conditions => ["transaction_date <= ?","#{query}"]}}
  named_scope :payment_mode_equals, lambda{|query|{:conditions => ["payment_mode = ?","#{query}"]}}
  named_scope :user_name_equals, lambda{|user_id|{:joins => :user,:conditions => ["finance_transactions.user_id = ?",user_id]}}
  named_scope :collection_id_equals,  lambda{|ftid|fid,ftype,fname = ftid.split(':');{:joins => "left outer join finance_fees on finance_fees.id=finance_transactions.finance_id left outer join transport_fees on transport_fees.id=finance_transactions.finance_id left outer join hostel_fees on hostel_fees.id=finance_transactions.finance_id left outer join instant_fees on instant_fees.id=finance_transactions.finance_id",:conditions => ["(finance_fees.fee_collection_id = ? and finance_transactions.finance_type = ?) or (transport_fees.transport_fee_collection_id = ? and finance_transactions.finance_type = ?) or (hostel_fees.hostel_fee_collection_id = ? and finance_transactions.finance_type = ?) or (instant_fees.instant_fee_category_id = ? and finance_transactions.finance_type = ?) or (instant_fees.instant_fee_category_id IS NULL and finance_transactions.finance_type = ? and instant_fees.custom_category=?)", fid,ftype,fid,ftype,fid,ftype,fid,ftype,ftype,fname]}}
  named_scope :employee_info_like,lambda{|search_string|{:joins => "left outer join employees es on es.id = finance_transactions.payee_id and finance_transactions.payee_type='Employee' left outer join archived_employees ars on ars.former_id = finance_transactions.payee_id and finance_transactions.payee_type='Employee'",:conditions=>["finance_transactions.payee_id in (?) or finance_transactions.payee_id in (?)",Employee.find(:all,:conditions=>["(ltrim(first_name) LIKE ? OR ltrim(middle_name) LIKE ? OR ltrim(last_name) LIKE ? OR employee_number = ? OR (concat(trim(first_name), \" \", trim(last_name)) LIKE ? ) OR (concat(trim(first_name), \" \", trim(middle_name), \" \", trim(last_name)) LIKE ? ))","%#{search_string}%","%#{search_string}%","%#{search_string}%","#{search_string}", "%#{search_string}%", "%#{search_string}%"]).collect(&:id).uniq.join(','),ArchivedEmployee.find(:all,:conditions=>["(ltrim(first_name) LIKE ? OR ltrim(middle_name) LIKE ? OR ltrim(last_name) LIKE ? OR employee_number = ? OR (concat(trim(first_name), \" \", trim(last_name)) LIKE ? ) OR (concat(trim(first_name), \" \", trim(middle_name), \" \", trim(last_name)) LIKE ? ))","%#{search_string}%","%#{search_string}%","%#{search_string}%","#{search_string}", "%#{search_string}%", "%#{search_string}%"]).collect(&:former_id).uniq.join(',')]}}
  named_scope :student_info_like,lambda{|search_string|{:joins => "left outer join students ss on ss.id = finance_transactions.payee_id and finance_transactions.payee_type='Student' left outer join archived_students ars on ars.former_id = finance_transactions.payee_id and finance_transactions.payee_type='Student'",:conditions=>["finance_transactions.payee_id in (?) or finance_transactions.payee_id in (?)",Student.find(:all,:conditions=>["(ltrim(first_name) LIKE ? OR ltrim(middle_name) LIKE ? OR ltrim(last_name) LIKE ? OR admission_no = ? OR (concat(trim(first_name), \" \", trim(last_name)) LIKE ? ) OR (concat(trim(first_name), \" \", trim(middle_name), \" \", trim(last_name)) LIKE ? ))","%#{search_string}%","%#{search_string}%","%#{search_string}%","#{search_string}", "%#{search_string}%", "%#{search_string}%"]).collect(&:id).uniq.join(','),ArchivedStudent.find(:all,:conditions=>["(ltrim(first_name) LIKE ? OR ltrim(middle_name) LIKE ? OR ltrim(last_name) LIKE ? OR admission_no = ? OR (concat(trim(first_name), \" \", trim(last_name)) LIKE ? ) OR (concat(trim(first_name), \" \", trim(middle_name), \" \", trim(last_name)) LIKE ? ))","%#{search_string}%","%#{search_string}%","%#{search_string}%","#{search_string}", "%#{search_string}%", "%#{search_string}%"]).collect(&:former_id).uniq.join(',') ]}}
