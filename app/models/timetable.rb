...
class Timetable < ActiveRecord::Base
  ...
  def self.tte_for_the_day(batch,date)
    ...
    entries = TimetableEntry.find(:all,:joins=>[:timetable, :class_timing], :conditions=>["timetable_entries.weekday_id = ? and (timetables.start_date <= ? AND timetables.end_date >= ?) AND timetable_entries.batch_id = ? AND class_timings.is_deleted = false",weekday,date,date,batch.id], :order=>"class_timings.start_time")
  ...
  def self.employee_tte(employee,date)
    ...
    entries = TimetableEntry.find(:all,:joins=>[:timetable, :class_timing, :employees],
      ...
      :conditions=>["(timetables.start_date <= ? AND timetables.end_date >= ?) AND 
                      ...
                      AND teacher_timetable_entries.employee_id = (?) AND class_timings.is_deleted = false",date,date,subjects.map(&:id),electives.map(&:id),employee.id],
