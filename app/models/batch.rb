...
class Batch < ActiveRecord::Base
 ...
 as eid,employees.first_name as ename,classrooms.name as cname,buildings.name as bname,ac.date as ddate",
        ...
        :joins=>"LEFT OUTER JOIN elective_groups eg on eg.id=timetable_entries.entry_id and timetable_entries.entry_type = 'ElectiveGroup'
                 ...
                 LEFT OUTER JOIN employees_subjects es on es.subject_id=s.id
      ...
      ct_hash.each do |ctid,v|
        ...
        subjects.each do |suid,v1|
          ...
          employees.each do |emid,v2|
            employee = v2.first
            hash[ctid][:subjects][suid][:employees][emid][:employee_name] = subject.timetable_swaps.present? ? (subject.timetable_swaps.first.employee.present? ? subject.timetable_swaps.first.employee.first_name : t('deleted_user')) : employee.ename
  ...
  def fetch_first_cce_exam_category
    ...
    first_cce_category_id=first_exam_group.first.cce_exam_category_id
  end
