class ReportController < ApplicationController
  ...
  def batch_fee_collections
    ...
    if FedenaPlugin.can_access_plugin?("fedena_transport") and FedenaPlugin.can_access_plugin?("fedena_hostel")
      @fee_collections = [
            {
              ...
              {
                :select =>"finance_fee_collections.id,finance_fee_collections.name,finance_fee_collections.start_date,finance_fee_collections.end_date,finance_fee_collections.due_date,sum(IF(students.id IS NOT NULL AND finance_fee_collections.is_deleted = false AND finance_fee_collections.due_date < '#{Date.today}' AND finance_fees.balance > 0.0,finance_fees.balance,0)) as balance,count(IF(finance_fees.balance!='0.0' and students.id IS NOT NULL,finance_fees.id,NULL)) as students_count",
                :joins => "INNER JOIN `finance_fees` ON finance_fees.fee_collection_id = finance_fee_collections.id INNER JOIN `batches` ON `batches`.id = `finance_fees`.batch_id INNER JOIN students on students.id=finance_fees.student_id",
                :conditions =>["finance_fees.batch_id=? and finance_fee_collections.is_deleted=? and finance_fee_collections.due_date < ? and finance_fees.balance> ?",params[:id],false,Date.today,0.0],
                :group => "id",
                :order => "balance DESC"
              }
            },
            {
              ...
              {
                :select => "transport_fee_collections.id,transport_fee_collections.name,transport_fee_collections.start_date,transport_fee_collections.end_date,transport_fee_collections.due_date,sum(IF(transport_fees.transaction_id is NULL and receiver_type='Student' and students.id IS NOT NULL,transport_fees.bus_fare,0)) as balance, count(DISTINCT IF(transport_fees.transaction_id is NULL and receiver_type='Student' and students.id IS NOT NULL,transport_fees.id,NULL)) as students_count",
                :joins => "INNER JOIN transport_fees on transport_fees.transport_fee_collection_id =transport_fee_collections.id INNER JOIN students on transport_fees.receiver_id=students.id and transport_fees.receiver_type='Student' INNER JOIN batches on students.batch_id=batches.id",
                :conditions =>["batches.id=? and transport_fee_collections.is_deleted=? and transport_fees.transaction_id IS NULL and transport_fee_collections.due_date < '#{Date.today}' and transport_fees.bus_fare > ? AND `transport_fees`.`is_active` = ?",params[:id],false,0.0,true],
            ...
            },
            {
              :hostel_fee_collection =>
                {
                  :select => "hostel_fee_collections.id,hostel_fee_collections.name,hostel_fee_collections.start_date,hostel_fee_collections.end_date,hostel_fee_collections.due_date,sum(IF(hostel_fees.finance_transaction_id is NULL and students.id IS NOT NULL ,hostel_fees.rent,0)) as balance, count(DISTINCT IF(hostel_fees.finance_transaction_id is NULL and students.id IS NOT NULL,hostel_fees.id,NULL)) as students_count",
                  :joins => "INNER JOIN hostel_fees on hostel_fees.hostel_fee_collection_id=hostel_fee_collections.id INNER JOIN students on hostel_fees.student_id=students.id INNER JOIN batches on students.batch_id=batches.id",
                  :conditions =>[
                    "batches.id=? and hostel_fee_collections.is_deleted=? and hostel_fees.finance_transaction_id IS NULL and hostel_fee_collections.due_date < ? and hostel_fees.rent > ? AND `hostel_fees`.`is_active` = ?",
                    params[:id],false,Date.today,0.0,true],
                  :group =>"hostel_fee_collections.id",
                  :order => "balance DESC"
                }
            }
      ].model_paginate(:page => params[:page], :per_page => 20)
      @total_amount+=TransportFeeCollection.sum(
                    "(IF(transport_fees.transaction_id is NULL and receiver_type='Student' and students.id IS NOT NULL and transport_fee_collections.is_deleted=false and transport_fee_collections.due_date < '#{Date.today}',transport_fees.bus_fare,NULL))",
                    :joins => "INNER JOIN transport_fees on transport_fees.transport_fee_collection_id = transport_fee_collections.id INNER JOIN students on students.id=transport_fees.receiver_id and transport_fees.receiver_type='Student' INNER JOIN batches on students.batch_id=batches.id",
                    :conditions =>["batches.id = ? AND `transport_fees`.`is_active` = ?",params[:id],true]
      ).to_f
      @total_amount+=HostelFeeCollection.sum(
                      "(IF(hostel_fees.finance_transaction_id is NULL and students.id IS NOT NULL and hostel_fee_collections.is_deleted =false and hostel_fee_collections.due_date < '#{Date.today}' ,hostel_fees.rent,NULL))",
                      :joins => "INNER JOIN hostel_fees on hostel_fees.hostel_fee_collection_id = hostel_fee_collections.id INNER JOIN students on students.id=hostel_fees.student_id INNER JOIN batches on students.batch_id=batches.id",
                      :conditions =>["batches.id = ? AND `hostel_fees`.`is_active` = ?",params[:id],true]
      ).to_f
      ...
      @fee_collections = [
        ...
        },
        {
          :hostel_fee_collection =>
          {
            :select => "hostel_fee_collections.id,hostel_fee_collections.name,hostel_fee_collections.start_date,hostel_fee_collections.end_date,hostel_fee_collections.due_date,sum(IF(hostel_fees.finance_transaction_id is NULL and students.id IS NOT NULL ,hostel_fees.rent,0)) as balance, count(DISTINCT IF(hostel_fees.finance_transaction_id is NULL and students.id IS NOT NULL,hostel_fees.id,NULL)) as students_count",
            :joins => "INNER JOIN hostel_fees on hostel_fees.hostel_fee_collection_id=hostel_fee_collections.id INNER JOIN students on hostel_fees.student_id=students.id INNER JOIN batches on students.batch_id=batches.id",
            :conditions =>["batches.id=? and hostel_fee_collections.is_deleted=? and hostel_fees.finance_transaction_id IS NULL and hostel_fee_collections.due_date < ? and hostel_fees.rent > ? AND `hostel_fees`.`is_active` = ?",params[:id],false,Date.today,0.0,true],
            :group =>"hostel_fee_collections.id",
            :order => "balance DESC"
          }
        }
      ...
      @total_amount+=HostelFeeCollection.sum(
        "(IF(hostel_fees.finance_transaction_id is NULL and students.id IS NOT NULL and hostel_fee_collections.is_deleted =false and hostel_fee_collections.due_date < '#{Date.today}' ,hostel_fees.rent,NULL))",
        :joins => "INNER JOIN hostel_fees on hostel_fees.hostel_fee_collection_id = hostel_fee_collections.id INNER JOIN students on students.id=hostel_fees.student_id INNER JOIN batches on students.batch_id=batches.id",
        :conditions =>["batches.id = ? AND `hostel_fees`.`is_active` = ?",params[:id],true]).to_f
      ...
      @total_amount+=TransportFeeCollection.sum(
          "(IF(transport_fees.transaction_id is NULL and receiver_type='Student' and students.id IS NOT NULL and transport_fee_collections.is_deleted=false and transport_fee_collections.due_date < '#{Date.today}',transport_fees.bus_fare,NULL))",
          :joins => "INNER JOIN transport_fees on transport_fees.transport_fee_collection_id = transport_fee_collections.id INNER JOIN students on students.id=transport_fees.receiver_id and transport_fees.receiver_type='Student' INNER JOIN batches on students.batch_id=batches.id",
          :conditions =>["batches.id = ? AND `transport_fees`.`is_active` = ?",params[:id],true]).to_f
    else
    ...
    if @sort_order.nil?
      if params[:transaction_class]=="HostelFeeCollection"
        @students=Student.paginate(
            :select => "students.roll_number,students.id,students.first_name,students.middle_name,students.last_name,students.admission_no,students.admission_date,rent as balance",
            :joins => [:hostel_fees],
            :conditions => [
              "hostel_fees.hostel_fee_collection_id=? and hostel_fees.finance_transaction_id is NULL and students.batch_id=? AND `hostel_fees`.`is_active` = ?", params[:collection_id],params[:id],true],
            :per_page => 20, :page => params[:page],
            :order => "balance DESC"
        )
        @total_amount=Student.sum("rent",
          :joins => [:hostel_fees],
          :conditions => [
            "hostel_fees.hostel_fee_collection_id=? and hostel_fees.finance_transaction_id is NULL and students.batch_id=? AND `hostel_fees`.`is_active` = ?",
            params[:collection_id],params[:id],true]
          )
      elsif params[:transaction_class]=="TransportFeeCollection"
        @students=Student.paginate(
          :select => "students.roll_number,students.id,students.first_name,students.middle_name,students.last_name,students.admission_no,students.admission_date,bus_fare as balance",
          :joins => "INNER JOIN transport_fees on transport_fees.receiver_id=students.id",
          :conditions => [
            "transport_fees.transport_fee_collection_id=? and transport_fees.transaction_id is NULL and students.batch_id=? AND `transport_fees`.`is_active` = ?",
            params[:collection_id],params[:id],true
          ],
          :per_page => 20,
          :page => params[:page],
        ...
        @total_amount=Student.sum("bus_fare",
          :joins => "INNER JOIN transport_fees on transport_fees.receiver_id=students.id",
          :conditions => [
            "transport_fees.transport_fee_collection_id=? and transport_fees.transaction_id is NULL and students.batch_id=? AND `transport_fees`.`is_active` = ?",
            params[:collection_id],params[:id],true
      ...
      if params[:transaction_class]=="HostelFeeCollection"
        @students=Student.paginate(
          :select => "students.roll_number,students.id,students.first_name,students.middle_name,students.last_name,students.admission_no,students.admission_date,rent as balance",
          :joins => [:hostel_fees],
          :conditions => ["hostel_fees.hostel_fee_collection_id=? and hostel_fees.finance_transaction_id is NULL and students.batch_id=? AND `hostel_fees`.`is_active` = ?", params[:collection_id],params[:id],true],
          :per_page => 20,
          :page => params[:page],
          :order => @sort_order)
        @total_amount=Student.sum("rent", :joins => [:hostel_fees], :conditions => ["hostel_fees.hostel_fee_collection_id=? and hostel_fees.finance_transaction_id is NULL and students.batch_id=? AND `hostel_fees`.`is_active` = ?", params[:collection_id],params[:id],true])
      elsif params[:transaction_class]=="TransportFeeCollection"
        @students=Student.paginate(
          :select => "students.roll_number,students.id,students.first_name,students.middle_name,students.last_name,students.admission_no,students.admission_date,bus_fare as balance",
          :joins => "INNER JOIN transport_fees on transport_fees.receiver_id=students.id",
          :conditions => ["transport_fees.transport_fee_collection_id=? and transport_fees.transaction_id is NULL and students.batch_id=?", params[:collection_id],params[:id]],
          :per_page => 20,
          :page => params[:page],
          :order => @sort_order)
        @total_amount=Student.sum("bus_fare",
        :joins => "INNER JOIN transport_fees on transport_fees.receiver_id=students.id", :conditions => ["transport_fees.transport_fee_collection_id=? and transport_fees.transaction_id is NULL and students.batch_id=?", params[:collection_id],params[:id]])
      else
  ...
  def student_wise_fee_collections
    ...
    if FedenaPlugin.can_access_plugin?("fedena_hostel") and FedenaPlugin.can_access_plugin?("fedena_transport")
      @total_amount+=HostelFeeCollection.sum("hostel_fees.rent",
                      :joins => [:hostel_fees],
                      :conditions => [
                        "hostel_fee_collections.due_date < ? AND `hostel_fee_collections`.`is_deleted` = ? AND `hostel_fees`.`student_id` = ? AND `hostel_fees`.`finance_transaction_id` IS NULL AND `hostel_fees`.`is_active` = ?",
                        Date.today,false,params[:id],true
                      ]
      ).to_f
      @total_amount+=TransportFeeCollection.sum("transport_fees.bus_fare",
                      :joins => "INNER JOIN transport_fees on transport_fees.transport_fee_collection_id = transport_fee_collections.id",
                      :conditions =>
                          [
                            "transport_fee_collections.is_deleted = ? AND `transport_fees`.`receiver_id` = ? AND `transport_fees`.`receiver_type` = 'Student' AND `transport_fees`.`transaction_id` IS NULL  AND `transport_fees`.`is_active` = ?",
                            false,params[:id],true
                          ]
      ).to_f
      @fee_collections = [
              {
                ...
                {
                  :select =>
                  "finance_fee_collections.name,finance_fee_collections.start_date,finance_fee_collections.end_date,finance_fee_collections.due_date,balance",
                  :joins => "LEFT OUTER JOIN finance_fees on finance_fees.fee_collection_id=finance_fee_collections.id INNER JOIN students on students.id=finance_fees.student_id",
                  :conditions => ["students.id=? and finance_fee_collections.is_deleted=? and finance_fee_collections.due_date < ? and finance_fees.balance > ?", params[:id], false,Date.today,0.0],
                  :order => 'balance DESC'
                }
              },
              {
                :hostel_fee_collection =>
                  {
                    :select => "name,start_date,end_date,due_date,hostel_fees.rent as balance",
                    :joins => [:hostel_fees],
                    :conditions => [
                      "`hostel_fee_collections`.`is_deleted` = ? AND `hostel_fees`.`student_id` = ? AND `hostel_fees`.`finance_transaction_id` IS NULL and hostel_fees.rent > ? AND hostel_fee_collections.due_date < ? AND `hostel_fees`.`is_active` = true",
                      false,params[:id],0.0,Date.today
                      ],
                    :order => "balance DESC"
                  }
              },
              {
                :transport_fee_collection =>
                  {
                    :select => "transport_fee_collections.id,name,start_date,end_date,due_date,transport_fees.bus_fare as balance",
                    :joins => "INNER JOIN transport_fees on transport_fees.transport_fee_collection_id = transport_fee_collections.id",
                    :conditions => [
                      "transport_fee_collections.is_deleted = ? AND `transport_fees`.`receiver_id` = ? AND `transport_fees`.`receiver_type` = 'Student' AND `transport_fees`.`transaction_id` IS NULL and transport_fees.bus_fare > ? AND transport_fee_collections.due_date < ? AND `transport_fees`.`is_active` = ?",
                      false,params[:id],0.0,Date.today,true],
                    :order => "balance DESC"
                  }
            }
          ].model_paginate(:page => params[:page], :per_page => 20)
      ...
      @total_amount+=HostelFeeCollection.sum("hostel_fees.rent",
                    :joins => [:hostel_fees],
                    :conditions => ["hostel_fee_collections.due_date < ? AND `hostel_fee_collections`.`is_deleted` = ? AND `hostel_fees`.`student_id` = ? AND `hostel_fees`.`finance_transaction_id` IS NULL AND hostel_fees.is_active = ?",
                      Date.today,false,params[:id],true
                      ]
      ).to_f
      @fee_collections = [
          {
            :finance_fee_collection =>
            {
              :select => "finance_fee_collections.name,finance_fee_collections.start_date,finance_fee_collections.end_date,finance_fee_collections.due_date,balance",
              :joins => "LEFT OUTER JOIN finance_fees on finance_fees.fee_collection_id=finance_fee_collections.id INNER JOIN students on students.id=finance_fees.student_id",
              :conditions => ["students.id=? and finance_fee_collections.is_deleted=? and finance_fee_collections.due_date < ? and finance_fees.balance > ?", params[:id], false,Date.today,0.0],
              :order => 'balance DESC'
            }
          },
          {
            :hostel_fee_collection =>
            {
              :select => "name,start_date,end_date,due_date,hostel_fees.rent as balance",
              :joins => [:hostel_fees],
              :conditions => ["`hostel_fee_collections`.`is_deleted` = ? AND `hostel_fees`.`student_id` = ? AND `hostel_fees`.`finance_transaction_id` IS NULL and hostel_fees.rent > ? AND hostel_fee_collections.due_date < ? AND `hostel_fees`.`is_active` = ?",false,params[:id],0.0,Date.today,true],
              :order => "balance DESC"
            }
          }
    ].model_paginate(:page => params[:page], :per_page => 20)
      ...
      @total_amount+=TransportFeeCollection.sum("transport_fees.bus_fare",
              :joins => "INNER JOIN transport_fees on transport_fees.transport_fee_collection_id = transport_fee_collections.id",
              :conditions => ["transport_fee_collections.is_deleted = ? AND `transport_fees`.`receiver_id` = ? AND `transport_fees`.`receiver_type` = 'Student' AND `transport_fees`.`transaction_id` IS NULL AND `transport_fees`.`is_active` = ?",false,params[:id],true]
      ).to_f
      @fee_collections = [
            {
              :finance_fee_collection =>
              ...
              },
              {
                :transport_fee_collection =>
                {
                  :select => "transport_fee_collections.id,name,start_date,end_date,due_date,transport_fees.bus_fare as balance",
                  :joins => "INNER JOIN transport_fees on transport_fees.transport_fee_collection_id = transport_fee_collections.id",
                  :conditions => ["transport_fee_collections.is_deleted = ? AND `transport_fees`.`receiver_id` = ? AND `transport_fees`.`receiver_type` = 'Student' AND `transport_fees`.`transaction_id` IS NULL and transport_fees.bus_fare > ? AND transport_fee_collections.due_date < ? AND `transport_fees`.`is_active` = ?",false,params[:id],0.0,Date.today,true],
                  :order => "balance DESC"
                }
              }].model_paginate(:page => params[:page], :per_page => 20)
    else
      @fee_collections=FinanceFeeCollection.paginate(
          :select => "finance_fee_collections.name,finance_fee_collections.start_date,finance_fee_collections.end_date,finance_fee_collections.due_date,balance",
          :joins => "LEFT OUTER JOIN finance_fees on finance_fees.fee_collection_id=finance_fee_collections.id INNER JOIN students on students.id=finance_fees.student_id",
          :conditions => ["students.id=? and finance_fee_collections.is_deleted=? and finance_fee_collections.due_date < ? and finance_fees.balance > ? ", params[:id], false,Date.today,0.0],
          :order => 'balance DESC',
  ...
  def batch_fees_headwise_report
    ...
    if request.xhr?
      if params[:batch_id].present?
        ...
        @students=Student.paginate(:select => "DISTINCT students.*", :conditions => ["finance_fees.batch_id IN (?)", batch_ids], :per_page => 10, :page => params[:page], :joins => [{:finance_fees=>{:finance_fee_collection=>:fee_collection_batches}}], :include => [{:finance_fees => [{:finance_fee_collection => [{:collection_particulars => :finance_fee_particular}, {:collection_discounts => :fee_discount}]}, :finance_transactions, :batch]}], :order => 'first_name ASC')
      elsif (course_id.blank?)
        @students=Student.paginate(:select => "DISTINCT students.*",:per_page => 10, :page => params[:page],:joins=>[{:finance_fees=>{:finance_fee_collection=>:fee_collection_batches}}], :include => [{:finance_fees => [{:finance_fee_collection => [{:collection_particulars => :finance_fee_particular}, {:collection_discounts => :fee_discount}]}, :finance_transactions, :batch]}], :order => 'first_name ASC')
      else
        @students=[]
      end
      render :update do |page|
  ...
  def student_fees_headwise_report_pdf
    @student=Student.find(params[:id])
    @batches=@student.all_batches.reverse
    render :pdf=> "student_fees_headwise_report_pdf", :margin =>{:left=>15,:right=>15,:top=>40,:bottom=>20}#,:header=>{:html=> {:template=> 'layouts/pdf_header.html.erb'}}  #  ,:show_as_html=>true
  end
  ...
  def fee_collection_head_wise_report
    ...
    if request.xhr?
      ...
      @students=Student.paginate(
        ...
        :conditions => ["finance_fees.fee_collection_id=? and finance_fees.batch_id=?", params[:fee_collection_id],params[:batch_id]],
        :per_page => 10,
        :page => params[:page],
        :joins => :finance_fees,
        :include => [{:finance_fees => [{:finance_fee_collection => [{:collection_particulars => :finance_fee_particular}, {:collection_discounts => :fee_discount}]}, :finance_transactions, :batch]}],
