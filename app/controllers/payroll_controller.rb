...
class PayrollController < ApplicationController
  ...
  def edit_category
    ...
    @categories = PayrollCategory.active_enabled(:all, :order=> "name ASC")
    @categories=@categories.reject { |category| (category.id == category_id)||(category.payroll_category_id == category_id)}
    @category = PayrollCategory.find(category_id)
  ...
  def manage_payroll
    ...
    payroll_created = EmployeeSalaryStructure.all(:conditions=>{:employee_id=>@employee.id,:payroll_categories=>{:status=>true,:is_deleted=>false}},:joins=>[:payroll_category])
  ...
  def update_dependent_fields
    ...
    @dependent_categories = PayrollCategory.active.find_all_by_payroll_category_id(cat_id,:conditions=>"status = true and is_deleted=false")
    @dependent_categories=@dependent_categories.reject { |category| category.id == cat_id}
  ...
  def edit_payroll_details
    @employee = Employee.find(params[:id])
    @independent_categories = PayrollCategory.active.find_all_by_is_deduction_and_status(false,true, :order=> "name ASC")
    @dependent_categories = PayrollCategory.active.find_all_by_is_deduction_and_status(true,true, :order=> "name ASC")
    if request.post?
      params[:manage_payroll].each_pair do |k, v|
        current_amount = v['amount'].to_f
