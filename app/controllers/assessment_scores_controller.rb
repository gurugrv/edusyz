...
class AssessmentScoresController < ApplicationController
  ...
  filter_access_to :all,:except=>[:observation_groups,:observation_scores]
  filter_access_to :only=>[:observation_groups,:observation_scores], :attribute_check=>true, :load_method => lambda { Batch.find(params[:batch_id]) }
  def exam_fa_groups
  ...
  def fa_scores
    ...
    @fa_criterias=@fa_group.fa_criterias.active         #.select{|m| m.descriptive_indicators.present?}
    ...
    if subject.elective_group_id.nil?
      if params[:student_order].present? && params[:student_order] == 'roll_number'
        @students=@batch.students.all(:order=>"LENGTH(roll_number) ASC,roll_number")
      else
        @students=@batch.students.all(:order=>"first_name ASC,last_name ASC")
      end
    else
      if params[:student_order].present? && params[:student_order] == 'roll_number'
        @students=subject.students.find(:all,:order=>"LENGTH(roll_number) ASC,roll_number")
      else
        @students=subject.students.find(:all,:order=>"first_name ASC,last_name ASC")
      end
    end
