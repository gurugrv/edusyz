class Api::StudentsController < ApiController
  ...
  def fee_dues
    ...
    DependencyHook.check_dependency_for(@student,:only => [:transport_batch_fee_value,:hostel_batch_fee_value]).each do |dependency|
      @hostel_fee_collections = dependency.result if dependency.hook_name == :hostel_batch_fee_value
      @transport_fee_collections = dependency.result if dependency.hook_name == :transport_batch_fee_value
    end
    respond_to do |format|
