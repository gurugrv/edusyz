class FinanceExtensionsController < FinanceController
  ...
  def fetch_all_fees
    ...
    master_fees_sql="SELECT distinct finance_fee_collections.name as collection_name,#{fee_transaction_category_id} as transaction_category_id,finance_fees.is_paid,finance_fees.balance,finance_fees.id as id,'FinanceFee' as fee_type,(finance_fees.balance+(select ifnull(sum(finance_transactions.amount-finance_transactions.fine_amount),0) from finance_transactions where finance_transactions.finance_id=finance_fees.id and finance_transactions.finance_type='FinanceFee')) as actual_amount,fine_rules.fine_amount as fine_amount,fine_rules.is_amount FROM `finance_fees` INNER JOIN `finance_fee_collections` ON `finance_fee_collections`.id = `finance_fees`.fee_collection_id INNER JOIN `fee_collection_batches` ON fee_collection_batches.finance_fee_collection_id = finance_fee_collections.id INNER JOIN `collection_particulars` ON (`finance_fee_collections`.`id` = `collection_particulars`.`finance_fee_collection_id`) INNER JOIN `finance_fee_particulars` ON (`finance_fee_particulars`.`id` = `collection_particulars`.`finance_fee_particular_id`) LEFT JOIN `fines` ON `fines`.id = `finance_fee_collections`.fine_id AND fines.is_deleted is false LEFT JOIN `fine_rules` ON fine_rules.fine_id = fines.id  and fine_rules.id=(select max(id) from fine_rules ffr where ffr.fine_id=fines.id and ffr.fine_days <= DATEDIFF(CURDATE(),finance_fee_collections.due_date))"
    (FedenaPlugin.can_access_plugin?("fedena_hostel") and (@current_user.admin? or @current_user.privileges.collect(&:name).include? "HostelAdmin"))? hostel_fees_sql="UNION ALL (SELECT fc.name as collection_name,#{hostel_transaction_category_id} as transaction_category_id,if(hf.finance_transaction_id is null,false,true) is_paid,if(hf.finance_transaction_id is null,hf.rent,0) balance,hf.id as id,'HostelFee' as fee_type,hf.rent actual_amount,0 fine_amount,0 is_amount FROM `hostel_fees` hf INNER JOIN `hostel_fee_collections` fc ON `fc`.id = `hf`.hostel_fee_collection_id and fc.is_deleted=0 and hf.student_id='#{@student.id}' and hf.finance_transaction_id is NULL and is_active is true)": hostel_fees_sql=''
    (FedenaPlugin.can_access_plugin?("fedena_transport") and (@current_user.admin? or @current_user.privileges.collect(&:name).include? "TransportAdmin"))? transport_fees_sql="UNION ALL (SELECT tc.name as collection_name,#{transport_transaction_category_id} as transaction_category_id,if(tf.transaction_id is null,false,true) is_paid,if(tf.transaction_id is null,tf.bus_fare,0) balance,tf.id as id,'TransportFee' as fee_type,tf.bus_fare actual_amount,0 fine_amount,0 is_amount FROM `transport_fees` tf INNER JOIN `transport_fee_collections` tc ON `tc`.id = `tf`.transport_fee_collection_id and tc.is_deleted=0 and tf.receiver_id='#{@student.id}' and tf.receiver_type='Student' and tf.transaction_id is NULL and is_active is true)" : transport_fees_sql=''
    @finance_fees=FinanceFee.find_by_sql("#{master_fees_sql} WHERE (finance_fees.student_id=#{@student.id} and finance_fees.is_paid=false and finance_fee_collections.is_deleted=0 and ((finance_fee_particulars.receiver_type='Batch' and finance_fee_particulars.receiver_id=finance_fees.batch_id) or (finance_fee_particulars.receiver_type='Student' and finance_fee_particulars.receiver_id=finance_fees.student_id) or (finance_fee_particulars.receiver_type='StudentCategory' and finance_fee_particulars.receiver_id=finance_fees.student_category_id))) GROUP BY finance_fees.id  #{transport_fees_sql}  #{hostel_fees_sql}")
  end
    ...
    if request.post?
      ...
      MultiFeesTransaction.transaction do
        ...
        if status and (multi_fees_transaction.valid? && finance_transactions.all?(&:valid?))
          ...
          tids=multi_fees_transaction.finance_transactions.collect(&:id)
          trans_code=[]
          tids.each do |tid|
  ...
  def get_paid_fees
    ...
    (FedenaPlugin.can_access_plugin?("fedena_hostel") and (@current_user.admin? or @current_user.privileges.collect(&:name).include? "HostelAdmin"))? fees=fees+",'HostelFee'" : fees
    (FedenaPlugin.can_access_plugin?("fedena_transport") and (@current_user.admin? or @current_user.privileges.collect(&:name).include? "TransportAdmin"))? fees=fees+",'TransportFee'" : fees
    transport_fee=" left join transport_fees on transport_fees.id=finance_transactions.finance_id and finance_transactions.finance_type='TransportFee' left join transport_fee_collections on transport_fee_collections.id=transport_fees.transport_fee_collection_id "
    hostel_fees=" left join hostel_fees on hostel_fees.id=finance_transactions.finance_id and finance_transactions.finance_type='HostelFee' left join hostel_fee_collections on hostel_fee_collections.id=hostel_fees.hostel_fee_collection_id "
    fees_combining=" left join finance_fees on finance_fees.id=finance_transactions.finance_id and finance_transactions.finance_type='FinanceFee' left join finance_fee_collections on finance_fee_collections.id=finance_fees.fee_collection_id"
    hostel_fee_sql=FedenaPlugin.can_access_plugin?("fedena_hostel") ? hostel_fees : ""
    transport_fee_sql=FedenaPlugin.can_access_plugin?("fedena_transport") ? transport_fee : ""
    fees_combining_sql=fees_combining+hostel_fee_sql+transport_fee_sql
    hostel_sql="','"
    transport_sql="','"
    hostel_sql="ifnull(hostel_fee_collections.name,'')" if FedenaPlugin.can_access_plugin?("fedena_hostel")
    transport_sql="ifnull(transport_fee_collections.name,'')" if FedenaPlugin.can_access_plugin?("fedena_transport")
    multi_fees_transactions_sql="SELECT distinct multi_fees_transactions.id,multi_fees_transactions.amount,multi_fees_transactions.created_at creation_time,concat(group_concat(ifnull(finance_fee_collections.name,'')),',',group_concat(#{hostel_sql}),',',group_concat(#{transport_sql})) as collection_name,concat(users.first_name,' ', users.last_name) as cashier,'multi_fees_transaction' as transaction_type,group_concat(finance_transactions.receipt_no) as receipt_no,finance_transactions.reference_no,finance_transactions.payment_mode,finance_transactions.payment_note,multi_fees_transactions.transaction_date FROM `multi_fees_transactions` INNER JOIN `multi_fees_transactions_finance_transactions` ON `multi_fees_transactions_finance_transactions`.multi_fees_transaction_id = `multi_fees_transactions`.id INNER JOIN `finance_transactions` ON `finance_transactions`.id = `multi_fees_transactions_finance_transactions`.finance_transaction_id left join users on users.id=finance_transactions.user_id #{fees_combining_sql}  WHERE (multi_fees_transactions.student_id='#{@student.id}') group by id "
    finance_transactions_sql="UNION ALL(SELECT finance_transactions.id,finance_transactions.amount,finance_transactions.created_at creation_time,if(finance_transactions.finance_type='FinanceFee',finance_fee_collections.name,if(finance_transactions.finance_type='HostelFee',#{hostel_sql},#{transport_sql})) as collection_name,concat(users.first_name,' ',users.last_name) as cashier,'normal_fees_transaction' as transaction_type,finance_transactions.receipt_no,finance_transactions.reference_no,finance_transactions.payment_mode,finance_transactions.payment_note,finance_transactions.transaction_date FROM `finance_transactions` LEFT OUTER JOIN `multi_fees_transactions_finance_transactions` ON `multi_fees_transactions_finance_transactions`.finance_transaction_id = `finance_transactions`.id LEFT OUTER JOIN `multi_fees_transactions` ON `multi_fees_transactions`.id = `multi_fees_transactions_finance_transactions`.multi_fees_transaction_id #{fees_combining_sql} left join users on users.id=finance_transactions.user_id WHERE (payee_id='#{@student.id}' and multi_fees_transactions.id is NULL and finance_type in ('FinanceFee','HostelFee','TransportFee')))"
