...
class UserController < ApplicationController
  ...
  filter_access_to [:profile,:user_change_password], :attribute_check=>true, :load_method => lambda { User.active.first(:conditions => ["username LIKE BINARY(?)",params[:id]]) }
  filter_access_to [:edit, :create, :index, :edit_privilege, :user_change_password,:delete,:list_user,:all_users]
    ...
    unless params[:query].nil? or params[:query].empty? or params[:query] == ' '
      ...
      @user = User.active.find(:all,
        :conditions => ["ltrim(first_name) LIKE ? OR ltrim(last_name) LIKE ?
                            OR username = ? OR (concat(ltrim(rtrim(first_name)), \" \", ltrim(rtrim(last_name))) LIKE ? ) ",
