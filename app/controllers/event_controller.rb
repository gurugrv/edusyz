...
class EventController < ApplicationController
  ...
  def index
    ...
    if request.post?
      if @events.save
        ...
        @start_date = params[:event][:start_date].to_time
        @end_date = params[:event][:end_date].to_time
      end
    end
  end
  ...
  def confirm_event
    ...
    reminder_body = " #{t('event_description')} : #{event.description} <br/> #{t('start_date')} : " + format_date(event.start_date) + " <br/> #{t('end_date')} : " + format_date(event.end_date)
    ...
    if event.is_common == true
      ...
      #          message = "#{t('event_text')} #{event.title} #{t('is_scheduled_from')} #{event.start_date} #{t('to_text')} #{event.end_date}. #{t('thanks')}"
    ...
    event.manual = true
    event.verify_update_and_send_sms
  ...
  def edit_event
    ...
    if request.post? and @event.update_attributes(params[:event])
      ...
      deliver_plugin_block :fedena_reminder do
        @event.set_alert_settings(params.fetch(:event,{})[:event_alerts_attributes])
      end
    end
  end
