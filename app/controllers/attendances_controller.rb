...
class AttendancesController < ApplicationController
  ...
  def subject_wise_register
    if params[:subject_id].present?
      ...
      respond_to do |fmt|
        fmt.json {render :json=>{'leaves'=>@leaves,'students'=>@students,'dates'=>@dates,'batch'=>@batch,'today'=>@today,'translated'=>@translated,'roll_number_enabled'=>Configuration.enabled_roll_number?}}
      end
    else
      render :update do |page|
        page.replace_html "register", :text => ""
  ...
  def daily_register
    ...
    if Configuration.enabled_roll_number?
      ...
      # @students = @batch.students.by_roll_number.with_full_name_roll_number
    else
    ...
    respond_to do |fmt|
      fmt.json {render :json=>{'leaves'=>@leaves,'students'=>@students,'dates'=>@dates,'holidays'=>@holidays,'batch'=>@batch,'today'=>@today, 'translated'=>@translated,'working_dates'=>@working_dates,'roll_number_enabled'=>Configuration.enabled_roll_number?}}
      #      format.js { render :action => 'show' }
    end
  end
  ...
  def destroy
    ...
    if @config.config_value=='Daily'
      ...
      @absentee = SubjectLeave.find(params[:id])
      @tte_entry = TimetableEntry.find_by_entry_id_and_class_timing_id(@absentee.subject_id,@absentee.class_timing_id)
