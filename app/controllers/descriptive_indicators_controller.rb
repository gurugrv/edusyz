...
class DescriptiveIndicatorsController < ApplicationController
  ...
  def show_in_report
    descriptive_indicator=DescriptiveIndicator.find(params[:di_id])
    descriptive_indicator.update_attribute('show_in_report',params[:show_in_report]) if params[:show_in_report].present?
    render :nothing=>true
  end
