...
class FinanceController < ApplicationController
  ...
  filter_access_to :refund_student_view,:refund_student_view_pdf,:attribute_check => true,:load_method => lambda {Student.find(params[:id])}
  ...
  def update_ajax
    ...
    render :update do |page|
      page.replace_html "fees_detail", :partial => "student_fees_submission"
      page.replace_html 'flash', :text=>"<p class='flash-msg'>#{flash[:warning]}</p>"
  ...
  def generate_fee_receipt_pdf
    if params[:particular_wise].present?
      ...
      render :pdf => 'generate_particular_fee_receipt_pdf',:template=>'finance/generate_particular_fee_receipt_pdf.erb',:margin =>{:top=>10,:bottom=>30,:left=>15,:right=>15},:header => {:html => { :content=> ''}}
    else
      get_student_fee_receipt(params[:transaction_id])
      render :pdf => 'generate_fee_receipt_pdf',:template=>'finance/generate_fee_receipt_pdf.erb',:margin =>{:top=>10,:bottom=>30,:left=>15,:right=>15},:header => {:html => { :content=> ''}}
    end
  end
  ...
  def fee_receipts
    ...
    unless params[:search].present?
      ...
      @end_date=Date.today
      @fee_receipts=fetched_fee_receipts.paginate(
        :per_page=>20,
        :page=>params[:page],
  ...
  def particular_and_discount_details
    @fee_particulars = @date.finance_fee_particulars.all(:conditions => "batch_id=#{@financefee.batch_id}").select { |par| (par.receiver_type=='Student' and par.receiver_id==@student.id)? par.receiver=@student : par.receiver;(par.receiver.present?) and (par.receiver==@student or par.receiver==@financefee.student_category or par.receiver==@financefee.batch) }
  ...
  def protect_access_by_other_students
    if current_user.student? or current_user.parent?
      ft=FinanceTransaction.find_by_id(params[:transaction_id])
