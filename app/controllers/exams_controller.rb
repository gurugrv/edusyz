...
class ExamsController < ApplicationController
  ...
  def edit
    ...
    @created_subject=@exam_group.exams.map{|m| m.subject_id}
    @created_subject.delete(@exam.subject_id)
    if @created_subject.empty?
       @subjects = @exam_group.batch.subjects.all
    else
      @subjects = @exam_group.batch.subjects.find(:all,:conditions=>["id not in (?)",@created_subject])
    end
#   @subjects=Subject.find(@exam.subject_id) if @subjects.empty?
    deliver_plugin_block :fedena_reminder do
  ...
  def show
    ...
    if is_elective == nil
      @students = @batch.students.by_full_name
    else
