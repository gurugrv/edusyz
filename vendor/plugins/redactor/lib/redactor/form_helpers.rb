...
#        "<input id='success_action_redirect' type='hidden' name='success_action_redirect' value='#{success_action_redirect}'>"
  ...
  def load_redactor_script
    ...
    unless(s3_present)
      ...
      image_upload_options = "imageUpload: 'https://#{Config.bucket_public}.#{server}/',
...
#       <script src='/javascripts/redactor/custom.js' type='text/javascript'></script>
  ...
  function init_redactor(b){
    j('#'+b.id).redactor({
      plugins: ['#{s3_image_upload_plugin}','fontcolor','#{latex_plugin}'],
      buttons: ['html', '|',
        ...
        'alignment', '|',
        'horizontalrule', '|',
        '#{image_button}', 'video', 'file', 'table', 'link'],
      buttonsCustom: {
        superscript: {
          title: 'Superscript',
