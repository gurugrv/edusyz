module FeeDefaultersSqlGenerator
  ...
  def sql_for_transport
    conditions = ["`fc`.`is_deleted` = 0","fc.due_date < '#{Date.today}'","ff.bus_fare>0","ff.transaction_id IS NULL "," ff.is_active = true"]
    conditions << "ff.school_id=#{MultiSchool.current_school.id}" if defined?(MultiSchool)
    where_condition = conditions.join(" AND ")
  ...
  def sql_for_hostel
    conditions = ["`fc`.`is_deleted` = 0","fc.due_date < '#{Date.today}'","ff.rent>0","ff.finance_transaction_id IS NULL "," ff.is_active = true"]
    conditions << "ff.school_id=#{MultiSchool.current_school.id}" if defined?(MultiSchool)
    where_condition = conditions.join(" AND ")
