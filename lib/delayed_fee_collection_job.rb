      ...
      FinanceFeeCollection.transaction do
        if @finance_fee_collection.save
          ...
          category.each do |b|
            ...
            @students = Student.find(:all,:conditions=>"batch_id=#{b} and has_paid_fees=0 and has_paid_fees_for_batch=0")
            ...
            body = "<p><b>#{t('fee_submission_date_for')} <i>"+fee_category_name+"</i> #{t('has_been_published')} </b>
              \n \n  #{t('start_date')}: "+format_date(@finance_fee_collection.start_date)+" \n"+
              " #{t('end_date')}: "+format_date(@finance_fee_collection.end_date)+" \n "+
              " #{t('due_date')}: "+format_date(@finance_fee_collection.due_date)+" \n \n \n "+
            ...
            recipient_ids.each do |r_id|
              Reminder.create(:sender => @user.id,:recipient => r_id,:subject => subject,:body => body)
            end
